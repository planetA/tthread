...
struct control_info {
  ...
  int bWrite;   /* Whether to write the private area in order to introduce
                   footprint on pages */
...
void *child_thread(void *data)
{
  ...
  if (pCntrl->conflict) {
    /* Write something to first page of global area in order to introduce some
       rollbak. */
    g_buffer[1] = i;
  }
...
void print_help(void)
{
  ...
  printf(
    "5, number of threads. Not a must, default is 8. It should be set when conflict rate is not 0\n");
}
...
int main(int argc, char **argv)
{
  ...
  const int thr_num = (argc == 6) ? atoi(argv[5]) : 16; /* default value for
                                                           number of threads. */
  ...
  for (i = 0; i < thr_num; i++)
  {
    struct control_info *control =
      (struct control_info *)((int)cntrl + i * sizeof(struct control_info));
  ...
  if (((thr_num * tran_size) > workload)
      || (workload % (thr_num * tran_size) != 0)) {
    printf(
      "Workload should be multiple times of the product of thread number and transaction size. \n");
    print_help();
    exit(1);
  }
