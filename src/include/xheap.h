...
public:
  ...
  // It is very very important to put the first page in a separate area since it
  // will invoke a
  // lot of un-necessary rollbacks, which can affect the performance greately
  // and also can affect
  // the correctness of race detection, since different threads can end up
  // getting the same memory
  // block from class "xheap".
  // Although we can rely on "rollback" mechanism to avoid the problem,
  // but we still need to differentiate those races caused by this and actual
  // races in user application.
  // It is not good to do this.
  // It is possible that we don't need sanityCheck any more, but we definitely
  // need a lock to avoid the race on "metatdata".
  xheap() {
    // Since we don't know whether shareheap has been initialized here, just use
    // mmap to assign
    // one page to hold all data. We are pretty sure that one page is enough to
    // hold all contents.
    char *base;
  ...
  // We need to page-aligned size, we don't need two different threads are using
  // the same page here.
  inline void *malloc(size_t sz) {
    ...
    if (*_remaining == 0) {
      fprintf(stderr,
              "Something very bad, maybe OUTOFMEMORY or memory corruption,  has happened: _start = %x, _position = %p, _remaining = %ld.\n",
              *_start,
              *_position,
              *_remaining);
      fprintf(stderr,
              "Try to change PROTECTEDHEAP_SIZE in xdefine.h to a bigger value\n");
      exit(-1);
    }

    if (*_remaining < sz) {
      fprintf(stderr,
              "OUTOFMEMORY: remaining[%ld], sz[%ld] thread[%d], try to change PROTECTEDHEAP_SIZE in xdefine.h to a bigger value\n",
              *_remaining,
              sz,
              (int)pthread_self());
      fprintf(stderr,
