...
double stop(struct timeinfo *begin,
            struct timeinfo *end);
...
# define STOP_TIMER(x) global_data->stats.x ## _total += \
  elapse2ms(stop(&global_data->stats.x ## _start, NULL))
# define PRINT_TIMER(x) fprintf(stderr,                                  \
                                " " # x " time: %.4fms\n",               \
...
# define PRINT_COUNTER(x) fprintf(stderr,                  \
                                  " " # x " count: %lu\n", \
