...
private:
  ...
  enum threadStatus {
    STATUS_COND_WAITING = 0, STATUS_BARR_WAITING, STATUS_READY, STATUS_EXIT,
    STATUS_JOINING
  };

  // Each thread has a thread entry in the system, it is used to control the
  // thread.
  // For example, when one thread is cond_wait, the corresponding thread entry
  // will be taken out of the token
...
public:
  ...
  // The following two flag are used to indentify whether one thread can move on
  // or not.
...
public:
  ...
  // pthread_cancel implementation, we are relying threadindex to find
  // corresponding entry.
  bool cancel(int threadindex) {
    ...
    case STATUS_COND_WAITING:
      ...
      // If the thread is waiting on condition variable, remove it from
      // corresponding list.
    {
    ...
    // If we can't find the entry, that means this thread has exited
    // successfully.
  ...
  void waitFence(int threadindex, bool keepBitmap) {
    ...
    if ((_maxthreads <= _coresNumb)
      ...
      while (_is_arrival_phase != true) {
        ...
        if (rv != 0) {
          fprintf(stderr,
                  "waitFence problem, can't issue condwait sucessfully\n");
          unlock();
          return;
        }
      }
    ...
    // Whenever all threads arrived in the barrier, wakeup everyone on the
    // barrier.
    if (_maxthreads <= _coresNumb) {
      if (_currthreads >= _maxthreads) {
        _is_arrival_phase = false;
  ...
  void putToken(int threadindex) {
    ...
    if (threadindex != _tokenpos->threadindex) {
      unlock();
      fprintf(stderr,
              "%d : ERROR to putToken, pointing to pid %d index %d, while my index %d\n",
              getpid(),
              _tokenpos->tid,
  ...
  void registerThread(int threadindex, int pid, int parentindex) {
    ...
    // _activelist->next is the first node in the link list, while
    // _activelist->prev is
  ...
  inline bool join(int guestindex, int myindex, bool wakeup) {
    ...
    // When the joinee is still alive, we should wait for the joinee to wake me
    // up
    ...
    while (joinee->status != STATUS_EXIT) {
      ...
      if ((_tokenpos->threadindex == myindex)
          && (_activelist != NULL)) {
      ...
      // put token later. For simplicity, all pthread_join should hold the
      // token.
      toWaitToken = true;
  ...
  void deregisterThread(int threadindex) {
    ...
    if ((parent->status == STATUS_JOINING)
        && (parent->joinee_thread_index == threadindex)) {
      // Move parent to the next, so that the parent can get the token
      // immediately.
      // Otherwise, if the token is passed to one thread (already finished the
      // token),
      // then the waken up thread cannot move on since it will wait for the
      // token to move on, however, the thread cannot pass the token and it is
      // waiting for fence now.
      insertHead((Entry *)parent, (Entry **)&_tokenpos);

      // Waken up all threads waiting on _cond_join, but only the one waiting on
      // this thread
  ...
  inline bool lock_acquire(void *mutex) {
    ...
    //  fprintf(stderr, "%d: lock acquire, with last thread %d, total users %d,
    // is_acquire %d\n", getpid(), entry->last_thread, entry->total_users,
    // entry->is_acquired);
    ...
    //  fprintf(stderr, "%d: lock acquire in the end, with last thread %d, total
    // users %d and is_acquire %d\n", getpid(), entry->last_thread,
    // entry->total_users, entry->is_acquired);
    return result;
  }
  ...
  void cond_wait(int threadindex, void *cond, void *thelock) {
    ...
    // We just need check whether all threads are inside the critical area or
    // not.
  ...
  void cond_signal(void *cond) {
    ...
    // token before all other threads
    // can get token. If one thread A before current thread has already passed
    // the token and are waiting on the fence
  ...
  int sig_wait(const sigset_t *set, int *sig, int threadindex) {
    ...
    if (_tokenpos == NULL) {
      ...
      // IMPORTANT: Add it to next of activelist, we still honor the previous
      // existing order.
      insertHead((Entry *)entry, (Entry **)&_tokenpos);
    }
...
private:
  ...
  // Second, it is safe to do so, since no one will change the entry until it is
  // destroyed.
