...
class Log:
    ...
    def read(self):
        ...
        for i in range(self.header.event_count):
            ...
            if type_byte >= len(events):
                raise Error("type field '%d' is out of range 0..%d" % (type_byte, len(events)))
    ...
    def _read_header(self):
        ...
        if stat.st_size < Header.size:
            raise Error("expected tthread_log to be at least %d, got %d" % (Header.size, stat.st_size))
        ...
        if header.file_magic != log_file_magic:
            Error("expect file_magick of tthread_log"
                    " to be equal %d, got %d" % (log_file_magick, header.file_magick))
        return header
    def close(self):
        self.file.close()
